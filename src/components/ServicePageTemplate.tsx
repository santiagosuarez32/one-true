    ...
    <main className="min-h-screen bg-white text-[#525252] selection:bg-[#FFC107] selection:text-[#411A56]" ref={containerRef}>
        ...
        <div className="w-full max-w-6xl lg:max-w-7xl xl:max-w-[1350px] mx-auto px-4 sm:px-8 md:px-12 lg:px-16 z-10 flex justify-start items-center">
          <div className="max-w-3xl text-left">
            ...
            {service.template === "poligrafo" && (
              <div className="flex flex-row items-center gap-6 sm:gap-10 mt-6 mb-6 border-t border-white/10 pt-6 w-full max-w-xl">
      ...
      <section className="bg-white py-20">
        <div className="w-full max-w-6xl lg:max-w-7xl xl:max-w-[1350px] mx-auto px-4 sm:px-8 md:px-12 lg:px-16">
          ...
          {/* Cards Grid */}
          {pageContent.aboutCardsLayout === "icon" ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
              {pageContent.aboutCards.map((card, idx) => {
                return (
                  <div key={idx} className="flex flex-col items-start gap-4 bg-white p-5 rounded border border-neutral-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] hover:shadow-[0_15px_35px_rgba(112,15,163,0.04)] transition-all duration-300 h-full w-full">
                    <div className="shrink-0">
                      {card.icon && (card.icon.startsWith("/") || card.icon.startsWith("http")) ? (
                        <img src={card.icon} alt={card.title} width={50} height={50} className="w-[50px] h-[50px] object-contain" />
                      ) : (
                        <img src={`/icons/eje${(idx % 3) + 1}.svg`} alt={card.title} width={50} height={50} className="w-[50px] h-[50px] object-contain" />
                      )}
                    </div>
                        ...
                        >
                          <span className="elementor-button-content-wrapper flex items-center justify-center gap-2" style={{ color: "#ffffff" }}>
                            ...
                            <span className="elementor-button-text font-bold" style={{ color: "#ffffff", fontWeight: "bold" }}>
                              {pageContent.contactWhatsappText || "+593 98 129 6179"}
                            </span>
                          </span>
                        </a>
                      </div>
      ...
      <Footer />
      <FloatingWhatsApp phone={pageContent.contactPhone} />
    </main>
  );
}
